...
namespace Intelica.Authentication.API.Controllers
{
    ...
    public class AuthenticateController(IGenericRSA customRSA, IAuthenticatorAggregate authenticator, IGenericCache cache, IEmailNotification emailNotification) : Controller
    {
        ...
        [HttpPost]
        [Route("ValidateLocalAuthentication")]
        public IActionResult ValidateLocalAuthentication(AuthenticationLocalQuery authenticationLocalQuery)
        {
            var ip = Request.Host.Value;
            var response = authenticator.ValidateAuthenticationLocal(authenticationLocalQuery, ip);
            return Ok(response);
        }
        [HttpPost]
        [Route("ValidateToken")]
        ...
        [HttpPost]
        [Route("ValidateTokenLocal")]
        public IActionResult ValidateTokenLocal(ValidateTokenLocalQuery validateTokenLocalQuery)
        {
            var userData = new UserDataRetriever(validateTokenLocalQuery.Token);
            var response = authenticator.ValidateTokenLocal(validateTokenLocalQuery.Token, userData.BussinesUserID, validateTokenLocalQuery.ClientID);
            return Ok(response);
        }
        [HttpPost]
